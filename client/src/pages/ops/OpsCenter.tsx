...
import { LayoutDashboard, FileText, Users, Activity, AlertTriangle, ArrowRight, TrendingUp, Menu, X } from "lucide-react";
...
export function OpsLayout({ children, activeHref }: { children: React.ReactNode; activeHref: string }) {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const { data: criticalExceptions } = trpc.ops.criticalExceptions.useQuery();
  const { data: health } = trpc.ops.systemHealth.useQuery();
  const ce = criticalExceptions as any[] ?? [];

  return (
    <div className="min-h-screen bg-[#1C1C1E] text-white flex flex-col md:flex-row">
      {/* Mobile header */}
      <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-white/10 bg-[#1C1C1E] sticky top-0 z-40">
        <div>
          <span className="text-sm font-semibold text-white/80">Lease</span>
          <span className="text-sm font-semibold text-[#4A7C7E]">Mate</span>
          <span className="text-xs text-white/30 ml-2">Ops</span>
        </div>
        <button
      ...
      {/* Mobile nav drawer */}
      {mobileNavOpen && (
        <div className="md:hidden bg-[#242424] border-b border-white/10 px-4 py-3">
          <nav className="space-y-1">
            {OPS_NAV.map((item) => (
              <Link key={item.href} href={item.href}>
                <div
                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                    activeHref === item.href ? "bg-white/10 text-white" : "text-white/50 hover:text-white/80 hover:bg-white/5"
                  }`}
      ...
      {/* Desktop sidebar */}
      <div className="hidden md:flex w-56 flex-shrink-0 border-r border-white/5 p-4 flex-col">
        ...
        {[
          ...
          <div key={stat.label} className="bg-white/5 rounded-xl border border-white/10 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-xs text-white/30 font-medium leading-tight">{stat.label}</div>
              <stat.icon size={14} className="text-white/20 flex-shrink-0" />
            </div>
          ...
          <div className="space-y-2">
            {ce.map((e: any) => (
              <Link key={e.id} href={`/ops/exceptions/${e.id}`}>
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-start md:items-center justify-between gap-3 cursor-pointer hover:bg-red-500/15 transition-colors">
                  <div className="flex items-start md:items-center gap-3 min-w-0">
                    <span className="text-xs font-mono font-semibold text-red-400 bg-red-500/20 px-2 py-0.5 rounded border border-red-500/30 flex-shrink-0">{e.code}</span>
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-white/80 truncate">{e.description}</div>
                      <div className="text-xs text-white/30 mt-0.5">{e.affectedParty}</div>
                    </div>
                  </div>
                  <ArrowRight size={14} className="text-red-400/60 flex-shrink-0 mt-1 md:mt-0" />
                </div>
              </Link>
            ))}
          </div>
