...
import { ArrowLeft, Clock, MapPin, Home, CheckCircle2, XCircle } from "lucide-react";
...
export default function ProviderOpportunities() {
  ...
  const acceptOpp = trpc.provider.acceptOpportunity.useMutation({
    onSuccess: () => { toast.success("Opportunity accepted. Proceed to payment."); refetch(); },
  ...
  const past = (opportunities as any[])?.filter((o: any) => o.status !== "pending") ?? [];
            ...
            {pending.map((opp: any) => (
              <div key={opp.id} className="bg-white rounded-xl border border-stone-200 p-5">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <div className="text-sm font-semibold text-[#2C2C2C] mb-1">{opp.categoryName}</div>
                    <div className="flex items-center gap-3 text-xs text-[#9B9B9B]">
                      <span className="flex items-center gap-1"><MapPin size={11} />{opp.suburb}</span>
                      <span className="flex items-center gap-1"><Home size={11} />{opp.propertyType} · {opp.bedrooms}bd {opp.bathrooms}ba</span>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm font-semibold text-[#2C2C2C]">Introduction fee: <span className="text-[#4A7C7E]">{opp.introductionFee ? `$${opp.introductionFee}` : "—"}</span></div>
            ...
            <div className="space-y-2">
              {past.map((opp: any) => (
                <div key={opp.id} className="bg-white rounded-xl border border-stone-100 p-4 flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-[#2C2C2C]">{opp.categoryName}</div>
                    <div className="text-xs text-[#9B9B9B]">{opp.suburb} · {opp.propertyType}</div>
                  </div>
